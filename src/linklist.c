...
int list_each_check_recv_complete(list *list, listIterator iterator){
  assert(iterator != NULL);
  ...
  if( result == TRUE){
	printf("All clients have got the ACK, lets move to the next frame\n");
	return 0;
  }
  else{
	printf("Not all clients has received ACKs.. need retransmission\n");
	return 1;
  }
}

 
void list_for_each_update_ack(list *list, paramlistIterator iterator,char* destination_ip)
{
  assert(iterator != NULL);
  ...
  bool result = TRUE;
  while(node != NULL && result) {
    result = iterator(node->data,destination_ip);
    node = node->next;
  }
}
void list_for_each_clear_reg(list *list, listIterator iterator)
{
  assert(iterator != NULL);
